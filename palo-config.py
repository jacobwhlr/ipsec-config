...
def main():
    ...
    for i in range(SUBNET_RANGE):
        ...
        on_boarding_file.write(f"set network ike gateway IP-{EXT_IP[i]} local-address ip {PALO_PUBLIC_IP} \n")
        ...
        on_boarding_file.write(f"set network ike gateway IP-{EXT_IP[i]} local-id id {PALO_PUBLIC_IP} \n")
        on_boarding_file.write(f"set network ike gateway IP-{EXT_IP[i]} local-id type ipaddr \n")
        on_boarding_file.write(f"set network ike gateway IP-{EXT_IP[i]} peer-id id {EXT_IP[i]} \n")
        on_boarding_file.write(f"set network ike gateway IP-{EXT_IP[i]} peer-id type ipaddr \n")
